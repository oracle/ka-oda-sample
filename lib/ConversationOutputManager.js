...
const ConversationOutputManager = {
	...
	sectionedArticleDetailToCardMessage: function(conversation, sectionList, articleURL) {
		...
		if (conversation.channelType() == "web") {
			if (sectionList.length > 1) {
				let section = sectionList[1]
				if (section.content.length > 128) {
					section.content = section.content.slice(0, 125) + "..."
				}
				cardList.push(MessageModel.cardObject(section.header, section.content, section.imageURL, undefined, [MessageModel.urlActionObject('View Full Article', undefined, articleURL)]))
			}
			return MessageModel.cardConversationMessage('vertical', cardList)
		} else {
